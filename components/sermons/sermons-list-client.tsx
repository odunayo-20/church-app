...
export function SermonsListClient({
  initialSermons,
  initialSeries,
}: {
  initialSermons: any;
  ...
  const isFiltering = search !== "" || selectedSeries !== "All";
  ...
  const { data: seriesData } = useSermonSeries();

  // If loading and not actively filtering on the client, fall back to initial server data
  const sermons = sermonsData?.data || (!isFiltering ? initialSermons?.data : []) || [];
  const series = seriesData || initialSeries || [];
  const isLoading = isSermonsLoading && isFiltering;
